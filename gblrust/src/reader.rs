...
<<<<<<< HEAD
        results.push(result.at(1).unwrap().to_string())        
=======
...
<<<<<<< HEAD
            ...
            None => Err("expected form, got EOF".to_string()),
        }        
    }
    ...
    fn read_delim(&mut self, left_token: &str, right_token: &str) -> Result<Vec<MalVal>, String> {        
        ...
        while self.peek() != Some(right_token.to_string()) {
            match self.peek() {
                Some(_) => match self.read_form() {
                    Ok(y) => output.push(Rc::new(y)),                   
                    Err(z) => return Err(z)
                },
                None => return Err(format!("expected '{}', got EOF", right_token).to_string())
=======
            },
...
>>>>>>> origin/master
    ...
    match r.peek() {
<<<<<<< HEAD
        Some(x) => Err(format!("Did not expect any more tokens at end of form while reading input but saw {}", x).to_string()),
        None => form        
=======
...
pub fn read_atom (token: &str) -> Result<MalType, String> {
    match token.chars().next() {
        Some(x) => 
            if x == '"' {
                Ok(MalType::MString(token[1..(token.len()-1)].to_string()))
=======
                ...
                match token {
                    ...
                    _ => {
                        ...
                        match try_parse {
                            Ok(x) => Ok(Integer(x)),
<<<<<<< HEAD
                            _ => Ok(Symbol(token.to_string()))                                                      
                        }
                    }
                }
            },                       
        None => Err("empty token".to_string())
    }
}  
